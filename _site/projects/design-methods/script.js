...
const closeBtn =
  modal?.querySelector(".close-button") || document.getElementById("close-btn");
    ...
    const methodRecords = await Promise.all(
      indexData.map(async (method) => {
        try {
          ...
          console.warn(`Could not load method record: ${method.id}`, error);
...
function showModal(method) {
  ...
  document.getElementById("method-description").innerHTML = formatInline(
    method.description || "",
  );
...
function updatePhaseFilter() {
  ...
  methodEntries.forEach((methodDiv) => {
    const methodStages =
      methodDiv.dataset.stages?.split(",").filter(Boolean) || [];

    const matches =
      noFilter || methodStages.some((stage) => activePhases.includes(stage));
...
function scoreMethod(method, query) {
  ...
  if (hasExactWord(method.name, normalizedQuery)) {
    score += 100;
  } else if (method.name.toLowerCase().includes(normalizedQuery)) {
    score += 70;
  } else if (fuzzyMatch(normalizedQuery, method.name.toLowerCase())) {
    score += 40;
  }
  ...
  if (hasExactWord(method.description, normalizedQuery)) {
    score += 30;
  }
  ...
  if (hasExactWord(method.whenToUse, normalizedQuery)) {
    score += 20;
  }
  ...
  if (
    ...
    method.lookOutFor.some((item) => hasExactWord(item, normalizedQuery))
  ) {
    score += 15;
  }
  ...
  if (
    ...
    method.steps.some((step) => hasExactWord(step, normalizedQuery))
  ) {
    score += 10;
  }
...
function filterMethods() {
  ...
  const rankedMethods = methodEntries
    .map((methodDiv) => {
      const method = methodData.get(methodDiv.dataset.id);
  ...
  rankedMethods.forEach((result) => {
    result.element.style.display = result.score > 0 ? "" : "none";
