export function render(root) {
  ...
  let count = 0;
  root.querySelector("#s02-send").addEventListener("click", () => {
    count++;
    const f = root.querySelector("#s02-feedback");
    if (count < 3) {
      f.textContent = "";
      f.hidden = true;
    } else {
      f.textContent =
        "You have requested too many recovery emails. Please try again later.";
      f.hidden = false;
    }
  });
  root
    .querySelector("#s02-account-link")
