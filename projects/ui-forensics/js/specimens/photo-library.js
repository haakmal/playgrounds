export function render(root) {
  ...
  root.querySelectorAll(".photo").forEach((photo) => {
    photo.addEventListener("click", () => {
      ...
      selected += photo.classList.contains("selected") ? 1 : -1;
    });
    let timer;
    photo.addEventListener("pointerdown", () => {
      timer = setTimeout(() => {
        if (selected >= 1) root.querySelector("#s31-move").hidden = false;
      }, 600);
    });
    ["pointerup", "pointerleave", "pointercancel"].forEach((type) =>
      photo.addEventListener(type, () => clearTimeout(timer)),
    );
  });
  root
    .querySelector("#s31-move")
    .addEventListener(
      "click",
      () => (root.querySelector("#s31-move").textContent = "Moved"),
    );
}
