export function render(root) {
  ...
  root.querySelectorAll('[name="q1"]').forEach((input) =>
    input.addEventListener("change", () => {
      root.querySelector("#s34-followup").hidden = false;
      root.querySelector("#s34-progress").textContent = "75% complete";
    }),
  );
  root
    .querySelector("#s34-back")
    .addEventListener("click", () =>
      root
        .querySelectorAll('[name="q1"]')
        .forEach((input) => (input.checked = false)),
    );
  root
    .querySelector("#s34-submit")
    .addEventListener(
      "click",
      () => (root.querySelector("#s34-progress").textContent = "100% complete"),
    );
}
