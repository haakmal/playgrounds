export function render(root) {
  ...
  root.querySelector("#s14-name").addEventListener("click", () => sort(1));
  root.querySelector("#s14-status").addEventListener("click", () => sort(2));
  root
    .querySelectorAll("[data-row]")
    .forEach((b) => b.addEventListener("click", () => {}));
  root.querySelector("#s14-bulk").addEventListener("click", () => {
    const n = root.querySelectorAll("[data-select]:checked").length;
    const f = root.querySelector("#s14-feedback");
    f.textContent = n ? `Action completed.` : "Action completed.";
    f.hidden = false;
  });
}
