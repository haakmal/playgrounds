export function createDebugController({ state, specimens, ui, elements }) {
  ...
  function updatePosition() {
    ...
    if (controls.next)
      controls.next.disabled = current >= state.specimens.length;
    if (controls.select)
      controls.select.value = state.currentSpecimen?.id || "";
  }

  function renderControls() {
    if (controls.select && !controls.select.options.length) {
      controls.select.innerHTML = specimens
        .map(
          (specimen) =>
  ...
  function next() {
    if (state.currentIndex < state.specimens.length - 1)
      render(state.currentIndex + 1);
  }

  function jump(specimenId) {
    const index = state.specimens.findIndex(
