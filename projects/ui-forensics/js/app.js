...
console.info(
  "%cUI FORENSICS NOTE%c Curious enough to inspect the tool? The hidden tutor reveal is Shift+R.",
  "font-weight:700",
  "font-weight:400",
);
...
async function initialise() {
  try {
    [specimenLibrary, issueLibrary] = await Promise.all([
      loadSpecimens(),
      loadIssues(),
    ]);
    ...
    ui = createInterface({
      ...
      callbacks: {
        onRenderCurrent: () => {
          tutor?.clear();
          elements.headerCase.textContent = debugMode
            ? "DEBUG / FULL LIBRARY"
            : `CASE ${state.caseId}`;
    ...
    document.querySelector("#app").innerHTML =
      `<section class="start-screen"><div class="start-copy"><p class="eyebrow mono">SYSTEM ERROR</p><h2>Specimen library unavailable.</h2><p>Run this project from a local web server or deploy it to a static host so the JSON data can be loaded.</p></div></section>`;
  }
}

if (elements.startButton)
  elements.startButton.addEventListener("click", startExamination);
if (elements.previousButton)
  elements.previousButton.addEventListener("click", previousSpecimen);
if (elements.resetButton)
  elements.resetButton.addEventListener("click", resetSpecimen);
if (elements.nextButton)
  elements.nextButton.addEventListener("click", nextSpecimen);
if (elements.newExaminationButton)
  elements.newExaminationButton.addEventListener("click", startExamination);
if (elements.redoSameButton)
  elements.redoSameButton.addEventListener("click", redoSameDeck);
