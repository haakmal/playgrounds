...
function scoreCandidate(candidate, selected) {
  ...
  const territoryCount = selected.filter(
    (item) => item.territory === candidate.territory,
  ).length;
  const deviceCount = selected.filter(
    (item) => item.device === candidate.device,
  ).length;
  const classCount = selected.filter(
    (item) => item.classification === candidate.classification,
  ).length;
  ...
  score -=
    candidate.classification === "dark-pattern" && classCount >= 2 ? 60 : 0;
...
export function chooseSpecimens(specimens, count = TARGET_COUNT) {
  ...
  if (hasMobile) {
    const mobile = shuffle(
      candidates.filter((item) => item.device === "mobile"),
    );
    selected.push(mobile[0]);
  }

  if (hasNonMobile && selected.length < count) {
    const nonMobile = shuffle(
      candidates.filter((item) => item.device !== "mobile"),
    );
    const candidate = nonMobile.find(
      (item) => !selected.some((existing) => existing.id === item.id),
    );
...
export function createCaseId() {
  ...
  let value = "";
  for (let i = 0; i < 6; i += 1)
    value += alphabet[randomValue(alphabet.length)];
  return value;
}
