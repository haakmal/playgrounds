export function createTutor({ state, issues, elements }) {
  ...
  function openIssue(issue) {
    const classification =
      issue.classification === "dark-pattern"
        ? ` · DARK PATTERN: ${issue.pattern.toUpperCase()}`
        : "";
  ...
  function revealCurrent() {
    ...
    state.currentIssues.forEach((issue, index) => {
      ...
      const pageX =
        ...
        (canvas.scrollWidth * issue.x) / 100;
      const pageY =
        canvas.getBoundingClientRect().top +
        (canvas.scrollHeight * issue.y) / 100;
  ...
  function closeDialog() {
    elements.issueDialog.hidden = true;
  }

  document.addEventListener("keydown", handleKeydown);
  document
    .querySelectorAll("[data-close-issue]")
    .forEach((element) => element.addEventListener("click", closeDialog));
