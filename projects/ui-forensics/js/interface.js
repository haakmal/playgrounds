...
export function createInterface({ state, issues, elements, callbacks }) {
  ...
  function getCurrentIssues() {
    ...
    return issues.filter((issue) =>
      issue.id.startsWith(`${state.currentSpecimen.id}-`),
    );
  }
  ...
  function renderCurrent() {
    ...
    elements.issueCount.textContent = String(specimen.issueCount).padStart(
      2,
      "0",
    );
    elements.specimenNumber.textContent = String(
      state.currentIndex + 1,
    ).padStart(2, "0");
    if (elements.specimenTotal) {
      elements.specimenTotal.textContent = String(
        state.specimens.length,
      ).padStart(2, "0");
    }
  ...
  function showComplete() {
    ...
    elements.completedList.innerHTML = state.specimens
      .map(
        (specimen, index) => `
