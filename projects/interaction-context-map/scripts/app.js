(function () {
  ...
  const SECTION_META = [
    [
      ...
      "Create the context for this mapping",
    ],
    [
      "observe",
      "02",
      "Observe an interaction",
      "Collect evidence without helping or correcting",
    ],
    [
      "capture",
      "03",
      "Capture the interaction",
      "Put observations into a simple sequence",
    ],
    [
      "ipo",
      "04",
      "Build an IPO map",
      "Look at the interaction from the perspective of the technology",
    ],
    [
      "ptc",
      "05",
      "Build a People / Technology / Context map",
      "Step back and observe the wider context",
    ],
    [
      "review",
      "06",
      "Connect and revisit the maps",
      "Compare what you have seen and what you now understand",
    ],
  ];
  ...
  const HELP = {
    ...
    input: {
      ...
      html: `<p>Inputs are things a person provides or does in the situation, such as clicking, tapping, typing, selecting, scanning, moving, speaking, or entering information.</p>`,
    },
    process: {
      title: "What is a Process?",
      html: `<p>The Process is what happens within the technology or system, such as information being searched, a selection being checked, a request being processed, information being retrieved, or a system state changing.</p><p><strong>Do not invent or infer the hidden technical process.</strong> Ask your tutor if you are not clear.</p>`,
    },
    output: {
      title: "What is an Output?",
      html: `<p>The Output is what the technology produces or communicates back to the person, such as information appearing, a message being displayed, a sound occurring, a light changing, a page changing, or an action being confirmed or rejected.</p>`,
    },
    people: {
      title: "Who are the People?",
      html: `<p>Ask who is involved in this situation, who is acting, who else is involved or affected, and what they are trying to accomplish.</p>`,
    },
    technology: {
      title: "What is the Technology?",
      html: `<p>Ask what they are interacting with. Is it a system, interface, information, object, service or mechanism? What does it allow the person to do?</p>`,
    },
    context: {
      title: "What is the Context?",
      html: `<p>Ask where and when the situation occurs and what is happening around the person. Are they rushed, distracted, interrupted or unfamiliar with the situation? Does the physical or social environment affect what they do?</p>`,
    },
  };
  ...
  function scheduleSave() {
    ...
    app.saveTimer = setTimeout(() => {
      ...
      setSaveState(
        "saved",
        `Saved locally · ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
      );
    }, 350);
  }
  ...
  function render() {
    ...
    workspace.innerHTML = SECTION_META.map(([key, n, title, summary]) =>
      renderSection(key, n, title, summary),
    ).join("");
    bindDynamicEvents();
  ...
  function isOpen(key) {
    return app.openSections.has(key);
  }
  ...
  function renderCapture() {
    ...
    const rowHtml = rows.length
      ? rows
          .map(
            (row, i) => `<tr data-observation-row="${row.id}">
      ...
      <td class="obs-row-actions"><div class="row-actions"><button class="small-button" data-edit-observation="${row.id}">Edit</button><button class="small-button danger" data-delete-observation="${row.id}">Del</button></div></td>
          ...
          .join("")
      : `<tr><td colspan="4"><div class="empty">No observations yet. Add your first row below.</div></td></tr>`;
    ...
    const columns = isIpo
      ? [
          ...
          [
            ...
            "Process",
            "process",
            "What happens within the technology/system",
          ],
          [
            "output",
            "Output",
            "output",
            "What the technology produces or communicates",
          ],
        ]
      : [
          ...
          [
            ...
            "Context",
            "context",
            "Where, when and what is happening around them?",
          ],
        ];
    const cols = columns
      .map(([key, title, help, hint]) => {
  ...
  function renderCard(card, mapType) {
    const selected =
      app.connectionMode.map === mapType &&
      app.connectionMode.sourceId === card.id;
  ...
  function renderReview() {
    ...
    const mapSummary = (map, keys) =>
      keys.flatMap((k) => map[k].map((c) => c.text).filter(Boolean));
    const previous = [
    ...
    const refs = previous.length
      ? `<ul class="review-list">${previous.map((t) => `<li>${escapeHtml(t)}</li>`).join("")}</ul>`
      : '<div class="empty">Your previous work will appear here as you build it.</div>';
    const prompts = [
    ...
    const reflections = app.project.review.reflections
      .map(
        (r) =>
          `<div class="review-item" data-reflection="${r.id}"><div class="review-date">${formatDate(r.createdAt)}</div><div>${escapeHtml(r.text)}</div><button class="small-button" data-delete-reflection="${r.id}">Delete</button></div>`,
      )
      .join("");
  ...
  function focusSection(key) {
    ...
    document.querySelectorAll("[data-focus-section]").forEach((btn) => {
      btn.textContent =
        focusing && btn.dataset.focusSection === key ? "Exit focus" : "Focus";
    });
  }
  function bindDynamicEvents() {
    ...
    document.querySelectorAll(".section").forEach((section) => {
      section.addEventListener("toggle", () => {
        ...
        if (section.open) app.openSections.add(key);
        else app.openSections.delete(key);
      });
    });
    document
      .querySelectorAll("[data-help]")
      .forEach((el) =>
        el.addEventListener("click", () => openHelp(el.dataset.help)),
      );
    ...
    document
      .querySelector("[data-add-observation]")
      ?.addEventListener("click", addObservation);
    document
      .querySelectorAll("[data-edit-observation]")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          editObservation(btn.dataset.editObservation),
        ),
      );
    document
      ...
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          deleteObservation(btn.dataset.deleteObservation),
        ),
      );
    document
      .querySelectorAll("[data-add-card]")
      .forEach((btn) =>
        btn.addEventListener("click", () => addMapCard(btn.dataset.addCard)),
      );
    document
      .querySelectorAll("[data-edit-card]")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          editMapCard(btn.dataset.map, btn.dataset.editCard),
        ),
      );
    document
      .querySelectorAll("[data-delete-card]")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          deleteMapCard(btn.dataset.map, btn.dataset.deleteCard),
        ),
      );
    document
      .querySelectorAll("[data-connect-map]")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          toggleConnectionMode(btn.dataset.connectMap),
        ),
      );
    ...
    document
      .querySelector("[data-add-reflection]")
      ?.addEventListener("click", addReflection);
    document
      .querySelectorAll("[data-delete-reflection]")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          deleteReflection(btn.dataset.deleteReflection),
        ),
      );
  }

  function addObservation() {
    app.project.observations.push({
  ...
  function deleteObservation(id) {
    ...
    app.project.observations = app.project.observations.filter(
      (r) => r.id !== id,
    );
    scheduleSave();
    render();
  ...
  function mapCollection(mapType, cardType) {
    return app.project[mapType][cardType];
  }
  ...
  function editMapCard(mapType, id) {
    ...
    document.getElementById("saveCard").addEventListener("click", () => {
      ...
      const text = document.getElementById("cardText").value.trim();
      Object.keys(app.project[mapType])
        .filter((k) => Array.isArray(app.project[mapType][k]))
        .forEach((k) => {
          if (k !== "connections")
            ICMMaps.removeCard(app.project[mapType][k], id);
        });
      if (text)
        ICMMaps.addCard(app.project[mapType][nextType], nextType, text).id = id;
      app.project[mapType].connections = app.project[
        mapType
  ...
  function toggleConnectionMode(mapType) {
    if (app.connectionMode.map === mapType) {
      app.connectionMode = { map: null, sourceId: null };
    } else {
      app.connectionMode = { map: mapType, sourceId: null };
    }
    render();
    openSection(mapType);
  }
  ...
  function handleCardConnection(mapType, cardId) {
    ...
    if (!app.connectionMode.sourceId) {
      app.connectionMode.sourceId = cardId;
      render();
      openSection(mapType);
      return;
    }
    ...
    if (source !== cardId) {
      ...
      const duplicate = connections.find(
        (c) =>
          (c.from === source && c.to === cardId) ||
          (c.from === cardId && c.to === source),
      );
      if (duplicate)
        app.project[mapType].connections = connections.filter(
          (c) => c !== duplicate,
        );
      else
        connections.push({ id: crypto.randomUUID(), from: source, to: cardId });
      scheduleSave();
    }
  ...
  function onDragStart(e) {
    if (app.connectionMode.map) {
      e.preventDefault();
      return;
    }
    app.drag = {
      cardId: e.currentTarget.dataset.cardId,
      mapType: e.currentTarget.dataset.map,
    };
  ...
  function onColumnDrop(e) {
    ...
    Object.keys(app.project[mapType])
      .filter(
        (k) => Array.isArray(app.project[mapType][k]) && k !== "connections",
      )
      .forEach((k) =>
        ICMMaps.removeCard(app.project[mapType][k], app.drag.cardId),
      );
  ...
  function addReflection() {
    ...
    if (!text) return;
    app.project.review.reflections.push({
      id: crypto.randomUUID(),
      text,
  ...
  function deleteReflection(id) {
    app.project.review.reflections = app.project.review.reflections.filter(
      (r) => r.id !== id,
    );
    scheduleSave();
    render();
  ...
  function showSessions() {
    ...
    openModal(
      "My interactions",
      `${html}<div style="display:flex;justify-content:space-between;gap:8px;margin-top:12px"><button class="button" data-action="new">New interaction</button><button class="button button-secondary" data-action="import">Import session</button></div>`,
    );
    document.querySelectorAll("[data-open-project]").forEach((btn) =>
      btn.addEventListener("click", () => {
        ICMStorage.setActive(btn.dataset.openProject);
        ensureProject();
        app.openSections = new Set(["setup", "observe", "capture"]);
        closeModal();
        render();
      }),
    );
    ...
    document.querySelectorAll("[data-duplicate-project]").forEach((btn) =>
      btn.addEventListener("click", () => {
    ...
    document.querySelectorAll("[data-delete-project]").forEach((btn) =>
      btn.addEventListener("click", () => {
        if (
          !confirm(
            "Delete this interaction from this browser? Export a backup first if you need it later.",
          )
        )
          return;
        ICMStorage.deleteProject(btn.dataset.deleteProject);
        ensureProject();
        closeModal();
        render();
      }),
  ...
  function exportSession() {
    ICMExport.downloadJson(app.project);
  ...
  window.addEventListener("beforeunload", () => {
    if (app.project) ICMStorage.saveProject(app.project);
  });
  ...
  setSaveState(
    "saved",
    app.project.modifiedAt
      ? `Saved locally · ${new Date(app.project.modifiedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
      : "Saved locally",
  );
})();
