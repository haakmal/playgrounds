...
document.getElementById("generate-scenario").onclick = async () => {
  const arcLine =
    document.querySelector("#arc-card").innerText.split("\n")[0] || "";
...
document.getElementById("provoke-scenario").onclick = async () => {
  ...
  const scenarioText = document
    .getElementById("scenario-output")
    .innerText.trim();
  ...
  try {
    ...
    if (!provoked) {
      document.getElementById("provoke-output").innerText =
        "No provocation received.";
      return;
    }

    stopGeneratingAnimation(provokeOutput); // stop animation
    document.getElementById("provoke-output").innerHTML =
      marked.parse(provoked);
  } catch (error) {
    console.error("Error fetching scenario:", error);
    stopGeneratingAnimation(provokeOutput); // stop animation
    document.getElementById("provoke-output").innerText =
      "Error fetching provocation.";
  }
};
