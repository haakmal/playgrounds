...
function loadState() {
  try {
    ...
    state.activeId =
      localStorage.getItem(ACTIVE_KEY) || saved?.activeId || null;
  } catch (err) {
    state.reports = {};
  }
  Object.values(state.reports).forEach(normaliseReport);
  if (!state.activeId || !state.reports[state.activeId]) {
    const first = Object.values(state.reports).sort((a, b) =>
...
function persist() {
  localStorage.setItem(
    STORAGE_KEY,
    JSON.stringify({
      schema: "ddes-usability-report/v2",
...
function currentReport() {
  return state.reports[state.activeId];
}
function touch() {
...
function buildLibraryFilters() {
  ...
  $("#sourceChips").innerHTML = sources
    .map(
      (s) =>
        `<button class="chip ${s === "all" ? "active" : ""}" data-library-source="${escapeHTML(s)}">${s === "all" ? "All sources" : escapeHTML(s)}</button>`,
    )
    .join("");
  const cats = [...new Set(HEURISTICS.map((h) => h.category))].sort();
  $("#libraryCategoryChips").innerHTML =
    '<button class="chip active" data-library-category="all">All categories</button>' +
    cats
      .map(
        (c) =>
          `<button class="chip" data-library-category="${escapeHTML(c)}">${escapeHTML(c)}</button>`,
      )
      .join("");
}
...
function validateContext() {
  ...
  required.forEach(([id, label]) => {
    ...
    const empty =
      !el ||
      !value ||
      (id === "reportTitle" && value === "Untitled usability report");
...
function findFinding(id) {
  return currentReport().findings.find((f) => f.id === id);
}
function currentEditingFinding() {
  return (
    state.editingFindingDraft ||
    (state.editingFindingId ? findFinding(state.editingFindingId) : null)
  );
}
...
function renderFindings() {
  ...
  $("#tableCount").textContent =
    `${arr.length} ${arr.length === 1 ? "finding" : "findings"} shown${arr.length !== currentReport().findings.length ? ` of ${currentReport().findings.length}` : ""}`;
  $$("#findingsTableBody [data-row-id]").forEach((row) =>
    row.addEventListener("click", () => openFindingEditor(row.dataset.rowId)),
  );
...
function openFindingEditor(target, focusLocation = false) {
  ...
  const h = HEURISTICS.find((x) => x.id === f.heuristicId);
  const related = (f.relatedIds || [])
    .map((x) => HEURISTICS.find((h) => h.id === x))
    .filter(Boolean);
...
function validateFinding(f) {
  ...
  if (heuristicError)
    heuristicError.textContent = missingHeuristic ? "Select a heuristic." : "";
  if (missingHeuristic) {
    valid = false;
...
function bindEditor(f) {
  $$(".severity-button").forEach((btn) =>
    btn.addEventListener("click", () => {
      f.severity = btn.dataset.severity;
      $$(".severity-button").forEach((x) =>
        x.classList.toggle("active", x === btn),
      );
      markEdited(f);
    }),
  );
  $("#edit-id").addEventListener("input", () => {
    f.id = $("#edit-id").value;
    clearFieldError("edit-id");
    markEdited(f);
  });
  [
    "edit-location",
  ...
  $("#findingEditor")
    .querySelector('[data-editor-action="save"]')
    .addEventListener("click", saveFinding);
  $("#findingEditor")
    .querySelector('[data-editor-action="cancel"]')
    .addEventListener("click", closeFindingEditor);
  $("#findingEditor")
    .querySelector('[data-editor-action="delete"]')
    .addEventListener("click", requestDeleteFinding);
  $("#findingEditor")
    .querySelector('[data-editor-action="library"]')
    .addEventListener("click", () => openLibrary(f));
}
...
function openLibrary(f) {
  ...
  $("#librarySearch").value = "";
  $$("#sourceChips .chip").forEach((x) =>
    x.classList.toggle("active", x.dataset.librarySource === "all"),
  );
  $$("#libraryCategoryChips .chip").forEach((x) =>
    x.classList.toggle("active", x.dataset.libraryCategory === "all"),
  );
...
function getFilteredHeuristics() {
  ...
  return HEURISTICS.filter((h) => {
    ...
    return (
      (!q || hay.includes(q)) &&
      (state.librarySource === "all" || h.source === state.librarySource) &&
      (state.libraryCategory === "all" || h.category === state.libraryCategory)
    );
...
function renderLibraryList() {
  ...
  if (
    ...
    !arr.some((h) => h.id === state.librarySelectedId)
  )
  ...
  list.innerHTML = arr.length
    ? arr
        .map(
          (h) =>
            `<div class="heuristic-list-item ${state.librarySelectedId === h.id ? "active" : ""}" data-library-id="${escapeHTML(h.id)}"><div class="source">${escapeHTML(h.source)}</div><div class="name">${escapeHTML(h.name)}</div><div class="category">${escapeHTML(h.category)}</div></div>`,
        )
        .join("")
    : '<div class="detail-empty">No matching heuristics.</div>';
  $$(".heuristic-list-item", list).forEach((x) =>
    x.addEventListener("click", () => {
      state.librarySelectedId = x.dataset.libraryId;
      renderLibraryList();
      renderLibraryDetail();
    }),
  );
...
function renderLibraryDetail() {
  ...
  if (!h) {
    detail.innerHTML = '<div class="detail-empty">Select a heuristic.</div>';
    $("#libraryContext").textContent = "";
    return;
  }
  $("#libraryContext").textContent = `${h.source} · ${h.category}`;
  const related = (h.related || [])
    .map((id) => HEURISTICS.find((x) => x.id === id))
    .filter(Boolean);
...
function renderReports() {
  ...
  const arr = Object.values(state.reports).sort((a, b) =>
    b.modified.localeCompare(a.modified),
  );
  list.innerHTML = arr
    .map(
      (r) =>
        `<div class="report-row"><div><div class="report-name">${escapeHTML(r.title)}${r.id === state.activeId ? " · open" : ""}</div><div class="report-meta-line">${r.findings.length} ${r.findings.length === 1 ? "finding" : "findings"} · edited ${formatDate(r.modified)}</div></div><div class="report-actions"><button class="button secondary" data-report-open="${escapeHTML(r.id)}">Open</button><button class="button secondary" data-report-rename="${escapeHTML(r.id)}">Rename</button><button class="button secondary" data-report-duplicate="${escapeHTML(r.id)}">Duplicate</button><button class="button secondary" data-report-delete="${escapeHTML(r.id)}">Delete</button></div></div>`,
    )
    .join("");
  $$("[data-report-open]", list).forEach(
    (b) =>
      (b.onclick = () => {
        state.activeId = b.dataset.reportOpen;
        state.editingFindingDraft = null;
        state.editingFindingId = null;
        state.currentFindingForLibrary = null;
        persist();
  ...
  $$("[data-report-rename]", list).forEach(
    (b) => (b.onclick = () => renameReport(b.dataset.reportRename)),
  );
  $$("[data-report-duplicate]", list).forEach(
    (b) => (b.onclick = () => duplicateReport(b.dataset.reportDuplicate)),
  );
  $$("[data-report-delete]", list).forEach(
    (b) =>
      (b.onclick = () => {
        state.reportToDelete = b.dataset.reportDelete;
...
function duplicateReport(id) {
  const src = state.reports[id];
  const name = prompt("Name the duplicate", `${src.title} copy`);
  if (name === null) return;
  const copy = JSON.parse(JSON.stringify(src));
  copy.id = uid("report");
  copy.title = name.trim() || `${src.title} copy`;
  copy.created = nowISO();
  copy.modified = copy.created;
  state.reports[copy.id] = copy;
...
function deleteConfirmed() {
  const id = state.reportToDelete;
  if (!state.reports[id]) return;
  ...
  if (!Object.keys(state.reports).length) {
    ...
    state.activeId = Object.keys(state.reports)[0];
...
function exportJSON() {
  ...
  downloadBlob(blob, `${slug(currentReport().title)}.json`);
  toast("JSON exported");
}
async function handleImport(e) {
  ...
  try {
    ...
    const src = normaliseReport(data.report || data);
    if (!src.findings || !src.metadata)
      throw new Error("Not a supported report");
    const copy = JSON.parse(JSON.stringify(src));
    copy.id = uid("report");
    copy.title = copy.title || "Imported usability report";
    ...
    alert("Could not import this file. Please use JSON exported by this tool.");
  } finally {
    e.target.value = "";
  }
}
...
function bindStaticEvents() {
  ...
  $("#interfaceHelpBtn").addEventListener("click", () =>
    openModal("interfaceHelpDialog"),
  );
  $("#reportToolsBtn").addEventListener("click", () =>
    openModal("reportToolsModal"),
  );
  $("#openReportsToolBtn").addEventListener("click", () => {
    closeModal("reportToolsModal");
    renderReports();
    openModal("reportsModal");
  });
  $("#newReportToolBtn").addEventListener("click", () => {
    closeModal("reportToolsModal");
    createReport();
  });
  ...
  $("#printToolBtn").addEventListener("click", () => {
    closeModal("reportToolsModal");
    printReport();
  });
  ...
  $("#confirmDeleteFindingBtn").addEventListener(
    "click",
    deleteFindingConfirmed,
  );
  $("#selectHeuristicBtn").addEventListener("click", applyLibrarySelection);
  ["reportTitle", "evaluator", "interfaceName", "goal"].forEach((id) =>
    $("#" + id).addEventListener("input", () => {
      updateMetadata();
      clearContextError(id);
    }),
  );
  $("#method").addEventListener("change", () => {
    updateMetadata();
    clearContextError("method");
  });
  [
    "findingSearch",
    "severityFilter",
    ...
    const el = $("#" + id);
    el.addEventListener("input", renderFindings);
    el.addEventListener("change", renderFindings);
  });
  $("#librarySearch").addEventListener("input", renderLibraryList);
  $("#sourceChips").addEventListener("click", (e) => {
    const b = e.target.closest("[data-library-source]");
    if (!b) return;
    state.librarySource = b.dataset.librarySource;
    $$("#sourceChips .chip").forEach((x) =>
      x.classList.toggle("active", x === b),
    );
    renderLibraryList();
  });
  $("#libraryCategoryChips").addEventListener("click", (e) => {
    const b = e.target.closest("[data-library-category]");
    if (!b) return;
    state.libraryCategory = b.dataset.libraryCategory;
    $$("#libraryCategoryChips .chip").forEach((x) =>
      x.classList.toggle("active", x === b),
    );
    renderLibraryList();
