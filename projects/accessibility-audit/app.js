(() => {
  ...
  const lenses = [
    {
      ...
      number: "01",
      title: "Access",
      question: "Can the interaction be perceived and reached at all?",
      guidance:
        "Can information be seen, heard, or otherwise perceived? Can controls be reached or activated? Is access dependent on one sense or ability?",
    },
    {
      key: "action",
      number: "02",
      title: "Action",
      question:
        "Can the user physically or cognitively perform the required actions?",
      guidance:
        "Look for motor and cognitive demands like precision, speed, or repetition. Is there an element of fatigue?",
    },
    {
      key: "understanding",
      number: "03",
      title: "Understanding",
      question: "Is it clear what is happening and what to do next?",
      guidance:
        "Look for cognition, language, and feedback cues. Are instructions clear? Is feedback present? Is there error tolerance? Does the interaction assume prior knowledge?",
    },
    {
      key: "adaptability",
      number: "04",
      title: "Adaptability",
      question:
        "Can the interaction adjust to different user needs or contexts?",
      guidance:
        "Look for resilience. Are there multiple ways to complete tasks? How about redundant modalities (visual + audio)? Is it compatible with assistive technologies?",
    },
    {
      key: "dignity",
      number: "05",
      title: "Dignity",
      question: "Does the interaction preserve autonomy and respect?",
      guidance:
        "Consider forced disclosures or dependence on staff, risk of public failure or embarrassment, and loss of agency.",
    },
  ];
  ...
  function renderEditorContent(key) {
    ...
    document
      .querySelectorAll(
        `.editable-content[data-editor-key="${CSS.escape(key)}"]`,
      )
      .forEach((element) => {
        ...
        if (content.trim()) {
          ...
          placeholder.textContent =
            key === "notes"
              ? "No further notes recorded. Select Edit to add notes."
              : `No observations recorded for ${title}. Select Edit to add observations.`;
  ...
  function updatePrintContext() {
    ...
    $("printFooterName").textContent =
      (a.name || "Untitled accessibility audit") +
      " · " +
      (a.interaction || "No interaction specified");
  ...
  function openAudits() {
    ...
    if (!audits.length) {
      ...
      list
        ...
        .forEach((button) =>
          button.addEventListener("click", () =>
            deleteAudit(button.dataset.delete),
          ),
        );
    }
  ...
  function exportAudits() {
    ...
    link.href = url;
    link.download =
      "accessibility-audits-" + new Date().toISOString().slice(0, 10) + ".json";
    link.click();
  ...
  $("newAuditModalBtn").addEventListener("click", () => {
    closeModal("auditsModal");
    createNewAudit();
  });
    ...
    try {
      ...
      const cleaned = incoming
        ...
        .map((a) => ({
          ...makeAudit(a.name),
          ...a,
          answers: { ...makeAudit(a.name).answers, ...(a.answers || {}) },
        }));
      if (!cleaned.length) throw new Error("No audits found");
      const replace = confirm(
        "Import " +
          cleaned.length +
          " audit(s). Choose OK to replace local audits, or Cancel to add them.",
      );
      ...
      alert(
        "Could not import this file. Please use a JSON export from this tool.",
      );
    }
    event.target.value = "";
  });
